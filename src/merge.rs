...
pub fn merge_pptx_files(input_files: &[impl AsRef<Path>], output: &Path) -> Result<()> {
    ...
    // Update docProps/app.xml with correct metadata
...
/// Update docProps/app.xml: slide count + theme count + titles.
fn update_app_xml(archive: &mut pptx::PptxArchive) {
    let total_slides = archive.keys()
        .filter(|k| k.starts_with("ppt/slides/") && k.ends_with(".xml") && !k.contains("/_rels/"))
        .count();
    let theme_count = archive.keys()
        .filter(|k| k.starts_with("ppt/theme/") && k.ends_with(".xml") && !k.contains("/_rels/"))
        .count();

    if let Some(app_xml) = archive.get("docProps/app.xml").cloned() {
        let mut xml_str = String::from_utf8_lossy(&app_xml).to_string();
        xml_str = replace_xml_tag(&xml_str, "Slides", &total_slides.to_string());

        // Update HeadingPairs theme count
        // Find <vt:i4>N</vt:i4> right after Theme entry and update it
        if let Some(theme_pos) = xml_str.find("<vt:lpstr>Theme</vt:lpstr>") {
            let after = &xml_str[theme_pos..];
            if let Some(i4_start) = after.find("<vt:i4>") {
                if let Some(i4_end) = after[i4_start..].find("</vt:i4>") {
                    let abs_start = theme_pos + i4_start;
                    let abs_end = theme_pos + i4_start + i4_end + "</vt:i4>".len();
                    let replacement = format!("<vt:i4>{}</vt:i4>", theme_count);
                    xml_str = format!("{}{}{}", &xml_str[..abs_start], replacement, &xml_str[abs_end..]);
                }
            }
        }

        // Update TitlesOfParts vector size to match theme count
        if let Some(tp_start) = xml_str.find("<TitlesOfParts>") {
            if let Some(vec_start) = xml_str[tp_start..].find("<vt:vector") {
                if let Some(size_start) = xml_str[tp_start + vec_start..].find("size=\"") {
                    let abs_start = tp_start + vec_start + size_start + "size=\"".len();
                    if let Some(quote_end) = xml_str[abs_start..].find('"') {
                        let abs_end = abs_start + quote_end;
                        xml_str = format!("{}{}{}", &xml_str[..abs_start], theme_count, &xml_str[abs_end..]);
                    }
                }
            }
        }

        archive.insert("docProps/app.xml".to_string(), xml_str.into_bytes());
    }
}

fn replace_xml_tag(xml: &str, tag: &str, value: &str) -> String {
    let open = format!("<{}>", tag);
    let close = format!("</{}>", tag);
    if let Some(start) = xml.find(&open) {
        let after_open = start + open.len();
        if let Some(end) = xml[after_open..].find(&close) {
            return format!("{}{}{}", &xml[..after_open], value, &xml[after_open + end..]);
        }
    }
    xml.to_string()
}
